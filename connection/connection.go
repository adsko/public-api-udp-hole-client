...
// TODO: Rename it to ServerClientConnection
...
func (c *connection) start (asServer bool, close chan string) {
...
func (c *connection) OnSend(cb func(data []byte, ip *net.UDPAddr) error) {
	c.sendFunc = cb
}

func (c *connection) StartServer(close chan string) {
...
func (c *connection) Send(data []byte, ip *net.UDPAddr) {
...
func (c *connection) connect(connected chan bool, asServer bool, close chan string) {
	sendCount := 0
	sendData := func() {
			...
			if asServer {
				...
				if err != nil {
					...
					if receiverIpAddr.IP.Equal(emptyIP.IP) {
						...
						log.Printf("esablished connection: %s | %s", sendOnIpAddr.IP.String(), receiverIpAddr.IP.String())
						goto CONNECTED
					}
				}
				break
...
func GetConnection(secret Secret, ips []IP) (Connection, error) {
